use {
    ...
    spl_stake_pool::instruction::{withdraw_sol, withdraw_sol_with_authority},
};
...
impl<'info> WithdrawSol<'info> {
    ...
    pub fn context_handler(ctx: Context<WithdrawSol>, pool_tokens_in: u64) -> Result<()> {
        let ix = if let Some(sol_withdraw_authority) = ctx.accounts.sol_withdraw_authority.clone() {
            let ix = withdraw_sol_with_authority(
