use {
    ...
    spl_stake_pool::instruction::{deposit_sol, deposit_sol_with_authority},
};
...
impl<'info> DepositSol<'info> {
    ...
    pub fn context_handler(ctx: Context<DepositSol>, lamports_in: u64) -> Result<()> {
        let ix = if let Some(sol_deposit_authority) = ctx.accounts.sol_deposit_authority.clone() {
            let ix = deposit_sol_with_authority(
        ...
        let mut account_infos: Vec<AccountInfo> = vec![
            ...
            // [s] Account providing the lamports to be deposited into the pool, --> should this also be writable?
